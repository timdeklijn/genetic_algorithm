...
impl Genes {
    fn new(genes: Vec<char>, size: usize) -> Genes {
        Genes { genes, size }
    }

    fn create_individual_genome(self) -> DNA {
        let dna = self
            .genes
            .choose_multiple(&mut rand::thread_rng(), self.size)
            .cloned()
            .collect();
        DNA::new(dna)
    }
...
impl DNA {
    fn new(dna: Vec<char>) -> Self {
        DNA { dna }
    }

    fn calc_fitness(&self) -> f32 {
        let mut f = 0;
        let d = self.dna.clone();
        for (i, gene) in d.into_iter().enumerate() {
            if gene == ANSWER.chars().nth(i).unwrap() {
                f += 1
            }
        }
        f as f32 / ANSWER.len() as f32
    }
}
...
impl Population {
    fn new(genes: Genes) -> Self {
        let mut pop = Vec::new();
        for _ in 0..POP_SIZE {
            let genes = genes.clone(); // TODO: Ugly, replace!
            let dna = genes.create_individual_genome();
            let i = Individual::new(dna);
            pop.push(i)
        }
...
fn main() {
    ...
    let gene_options: Vec<char> = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ,.?!'; "
        .chars()
        .collect();
    // Create Genes object
    let genes = Genes::new(gene_options, ANSWER.len());
    // Create population
    let mut population = Population::new(genes);
    // Run the algorithm
    population.run();
}
